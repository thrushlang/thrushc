...
fn parse_pe_header(path: &Path) -> io::Result<(u16, u16)> {
    ...
    file.seek(start)?;
    file.read_exact(&mut buffer)?;
    let offset: i32 = i32::from_le_bytes(buffer);
    ...
    file.read_exact(&mut buffer)?;
    let machine_type: u16 = u16::from_le_bytes(buffer);
...
fn search_libclang_directories(runtime: bool) -> Result<Vec<(PathBuf, String, Vec<u32>)>, String> {
    let mut files: Vec<String> = vec![format!(
        "{}clang{}",
    ...
    let mut valid: Vec<(PathBuf, String, Vec<u32>)> = vec![];

    let mut invalid = vec![];
    for (directory, filename) in common::search_libclang_directories(&files) {
