...
pub fn compile<'ctx>(
    ...
    let indexes: Vec<IntValue> = if is_ptr_aggv_type {
        let base: IntValue = integer::generate(context, &Type::U32, 0, false, index.get_span());
        let depth: IntValue = codegen::compile(context, index, Some(&Type::U32)).into_int_value();

        vec![base, depth]
    } else if is_ptr_like_type {
        let base: IntValue = codegen::compile(context, index, Some(&Type::U64)).into_int_value();
